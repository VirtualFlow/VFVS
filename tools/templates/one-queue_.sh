...
update_summary() {
    ...
    if [ ! -f /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt ]; then
        printf "Compound   average-score   maximum-score   number-of-dockings" >> /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt
        for k in $(seq 1 ${docking_replica_index_end}); do 
            printf "   score-replica-$k" >>  /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt
        done
        printf "\n" >> /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt
    fi
    if [ "${docking_replica_index}" -eq "1" ]; then
        printf "${next_ligand} %3.1f %3.1f %5s %3.1f\n" "${score_value}" "${score_value}" "1" "${score_value}" >> /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt
    else
        scores_previous=$(grep ${next_ligand} /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt | tr -s " " | cut -d " " -f 5-)
        ...
        perl -pi -e "s/${next_ligand}\b.*/${next_ligand} ${score_average} ${score_maximum} ${docking_replica_index} ${scores_all_expaned}/g" /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt
    fi
    column -t /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt > /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt.tmp
    mv /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt.tmp /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt
}
...
prepare_collection_files_tmp() {
    ...
    for docking_type_name in ${docking_type_names[@]}; do
        ...
        if [ ! -d "/tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/" ]; then
            mkdir -p /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/
        elif [ "$(ls -A "/tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/")" ]; then
            rm -r /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/*
        fi
    ...
    for docking_type_name in ${docking_type_names[@]}; do
        if [ "${new_collection}" = "false" ]; then
            ...
            if [[ -f "../output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt" ]]; then
                cp ../output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_basename}.txt /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/
            fi
        fi
    ...
    if [ ${needs_cleaning} = "true" ]; then
        ...
        for docking_type_name in ${docking_type_names[@]}; do
            if [ "${collection_complete}" = "true" ]; then
                ...
                if [ ! -d "../output-files/complete/${docking_type_name}/summaries/" ]; then
                    mkdir -p ../output-files/complete/${docking_type_name}/summaries/
                fi
                ...
                mkdir -p /tmp/${USER}/${queue_no}/output-files/complete/${docking_type_name}/summaries/${local_ligand_collection_sub1}
                cp /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${local_ligand_collection_basename}.txt /tmp/${USER}/${queue_no}/output-files/complete/${docking_type_name}/summaries/${local_ligand_collection_sub1}/${local_ligand_collection_no_basename}.txt || true
                gzip -f /tmp/${USER}/${queue_no}/output-files/complete/${docking_type_name}/summaries/${local_ligand_collection_sub1}/${local_ligand_collection_no_basename}.txt || true
                tar -rf ../output-files/complete/${docking_type_name}/summaries/${local_ligand_collection_sub1}.tar -C /tmp/${USER}/${queue_no}/output-files/complete/${docking_type_name}/summaries/ ${local_ligand_collection_sub1}/${local_ligand_collection_no_basename}.txt.gz || true
                ...
                rm -r ../output-files/incomplete/${docking_type_name}/summaries/${local_ligand_collection_basename}* &>/dev/null || true
            else
                # Copying the files which should be kept in the permanent storage location
                ...
                if [ -f /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${local_ligand_collection_basename}.txt ]; then
                    if [ ! -d "../output-files/incomplete/${docking_type_name}/summaries/" ]; then
                        mkdir -p ../output-files/incomplete/${docking_type_name}/summaries/
                    fi
                    cp /tmp/${USER}/${queue_no}/output-files/incomplete/${docking_type_name}/summaries/${local_ligand_collection_basename}.txt ../output-files/incomplete/${docking_type_name}/summaries/
                fi
            fi
        done

...
update_summary() {
    ...
    if [ "${docking_replica_index}" -eq "1" ]; then
        ...
        perl -pi -e "s/${next_ligand_collection} *${next_ligand}\b.*/${next_ligand_collection_tranch} ${next_ligand} ${score_average} ${score_maximum} ${docking_replica_index} ${scores_all_expaned}/g" ${VF_TMPDIR}/${USER}/VFVS/${VF_JOBLETTER}/${VF_QUEUE_NO_12}/${VF_QUEUE_NO}/output-files/incomplete/${docking_type_name}/summaries/${next_ligand_collection_metatranch}/${next_ligand_collection_tranch}/${next_ligand_collection_ID}.txt
    fi
